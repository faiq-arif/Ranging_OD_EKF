...
def measJacobian(X_gps_t, X_t):
    ...
    return J
'''
# Define the measurement functions and Jacobians
def rangeMeas(gs_positions, sat_position):
    return np.linalg.norm(gs_positions - sat_position[:3], axis=1)

def measurements(gs_positions, sat_position):
    ranges = rangeMeas(gs_positions, sat_position)
    # No need to compute verificator range separately; directly return all four ranges
    return ranges

def rangeMeasJacobian(gs_positions, sat_position):
    num_gs = gs_positions.shape[0]
    norm = np.linalg.norm(gs_positions - sat_position[:3], axis=1, keepdims=True)
    C_r = (sat_position[:3] - gs_positions) / norm
    C_r = np.concatenate((C_r, np.zeros((num_gs, 3))), axis=1)
    return C_r

def measJacobian(gs_positions, sat_position):
    return rangeMeasJacobian(gs_positions, sat_position)
