...
# EKF function
def EKF_aggregate(X, Y, mu0, Sigma0, Q, R, fDynamics, JDynamics, propagator, dt, fMeas, JMeas, gs_positions=None):
    T, n = X.shape
    mu = np.zeros((T, n))
    ...
    for t in range(1, T):
        ...
        Sigma[t,:,:] = np.dot(A[t-1,:,:], np.dot(Sigma[t-1,:,:], A[t-1,:,:].T)) + Q
        A[t,:,:] = JDynamics(mu[t,:])

        if gs_positions is None or JMeas is None:
            C = np.eye(n)
            y = fMeas(C, mu[t,:])
            ...
            y = fMeas(gs_positions[t], mu[t,:])
            C = JMeas(gs_positions[t], mu[t,:])
            Y_aggregate = Y[t,:].flatten()

            K = np.dot(Sigma[t,:,:], np.dot(C.T, np.linalg.inv(np.dot(C, Sigma[t,:,:]).dot(C.T) + R)))
            mu[t,:] += np.dot(K, Y_aggregate - y)
            Sigma[t,:,:] -= np.dot(K, np.dot(C, Sigma[t,:,:]))
